...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'nf-core-circrnasponging-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/circrnasponging Workflow Summary'
...
workflow.onComplete {
    ...
    def subject = "[nf-core/circrnasponging] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/circrnasponging] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/circrnasponging] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/circrnasponging] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/circrnasponging] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            ...
            log.info "[nf-core/circrnasponging] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "-${c_purple}[nf-core/circrnasponging]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
        log.info "-${c_purple}[nf-core/circrnasponging]${c_red} Pipeline completed with errors${c_reset}-"
    }
...
def nfcoreHeader() {
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/circrnasponging v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
